...
import org.oasis.tosca.TBoundaryDefinitions;
...
import at.ac.tuwien.dsg.comot.common.fluent.ArtifactReference;
import at.ac.tuwien.dsg.comot.common.fluent.ArtifactTemplate;
import at.ac.tuwien.dsg.comot.common.fluent.BundleConfig;
import at.ac.tuwien.dsg.comot.common.fluent.Capability;
import at.ac.tuwien.dsg.comot.common.fluent.CloudEntity;
...
import at.ac.tuwien.dsg.comot.common.fluent.OperatingSystemSpecification;
...
public class ToscaDescriptionBuilderImpl implements ToscaDescriptionBuilder {
	...
	public TDefinitions buildToscaDefinitions(CloudService cloudService) throws ToscaDescriptionBuilderException {
		...
		List<TExtensibleElements> tServiceTemplates = definitions
		...
		for (ServiceTopology concept : cloudService.getServiceTopologies()) {
			...
			TServiceTemplate tServiceTemplate = new TServiceTemplate().withId(concept.getId()).withName(
					concept.getName());
		...
		if (!unresolvedRelationships.isEmpty()) {
			for (Map.Entry<EntityRelationship, TTopologyTemplate> unresolvedRelationship : unresolvedRelationships
					.entrySet()) {
		...
		// definitions.getOtherAttributes().put(new QName(DEFAULT_ARTIFACT_TYPE_NAMESPACE_URI, "defaultMetricsEnabled",
		// DEFAULT_ARTIFACT_TYPE_PREFIX), "true");
		// }
		return definitions;
	...
	private TRelationshipTemplate buildTRelationshipTemplate(EntityRelationship relationship,
			boolean throwOnMissingRelationship) {
	...
	private List<TRelationshipTemplate> extractRelationships(CloudService serviceTemplate,
