define(function(require) {
	var app = require('durandal/app'), d3 = require('d3');

	console.log("aaaaaaaaaaaaaaaaaa " + d3.version);
	...
	function createTree() {
		...
		var svg = d3.select(".tree_div").append("svg").attr("width", width).attr("height", height).append("g").attr(
		...
		d3.json("flare.json", function(error, root) {
			...
			node.append("text").attr("dx", function(d) {
				return d.children ? -8 : 8; // right vs. left side
			...
			}).attr("dy", 15).style("text-anchor", function(d) {
				return d.children ? "end" : "start";
			}).text(function(d) {
				return "aaa";
			});
